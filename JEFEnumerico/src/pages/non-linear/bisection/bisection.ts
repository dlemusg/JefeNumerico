...
export class BisectionPage {
  ...
  graficador() {
    if (this.dataReceived.n.length <= 0) this.submitForm();
    else {
      var a: string = this.dataSubmit['xa'] + "";
      var b: string = this.dataSubmit['xb'] + "";

      if (this.dataReceived['aproximados'] != undefined && 
      this.dataReceived['aproximados'] != "" && 
      this.dataSubmit['fx'] != "" && this.dataSubmit['xa'] != "" && 
      this.dataSubmit['xb'] != "") {
        var aux: number = <number><any>this.dataReceived['aproximados'];
        a = "" + (aux - 0.2);
        b = "" + (aux - (-0.2));
      }
